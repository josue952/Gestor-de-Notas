...
public function store(Request $request)
{
    ...
    // Verificar si ya existe un estudiante con el mismo usuario_id en la misma clase
    $existingStudentInSameClassType = Estudiantes::where('usuario_id', $request->usuario_id)
        ->whereHas('clase', function ($query) use ($request) {
            $query->where('nombre', Clases::find($request->clase_id)->nombre);
        })
        ->first();

    if ($existingStudentInSameClassType) {
        return response()->json([
            'error' => 'El estudiante ya está inscrito en otra clase de este tipo.'
        ], 400);
    }
